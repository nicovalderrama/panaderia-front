...
export default function Reportes() {
  ...
  const headers = ['ID', 'Fecha de venta', 'Cliente', 'Forma de pago', 'Usuario', 'Monto total'];
  const clearFilter = () =>{
    setCategoryFilter('');
    setProductFilter('');
    setStartDate('');
  ...
  const filterVentas = () => {
    ...
    if (productFilter) {
      filtered = filtered.filter(venta => 
        venta.items?.some(item => item.producto_nombre.toLowerCase().includes(productFilter.toLowerCase()))
      );
    }

    if (categoryFilter) {
      filtered = filtered.filter(venta => 
        venta.items?.some(item => item.producto_categoria.toLowerCase().includes(categoryFilter.toLowerCase()))
      );
    }

    if (startDate) {
      filtered = filtered.filter(venta => new Date(venta.fecha_venta) >= new Date(startDate));
    }

    if (endDate) {
      filtered = filtered.filter(venta => new Date(venta.fecha_venta) <= new Date(endDate));
    }
    ...
    if (sortBy === "most_sold") {
      ...
      filtered.sort((a, b) => new Date(b.fecha_venta).getTime() - new Date(a.fecha_venta).getTime());
    }
  ...
  return (
    <DashboardPage>
      <div className="flex flex-col p-4 bg-base-100 rounded">
        <h1 className="text-2xl font-bold m-8">Reportes ventas</h1>
        ...
        <div className="flex flex-wrap p-4 rounded-xl shadow gap-4 mb-4 bg-gray-50">
          ...
          <button onClick={clearFilter} className="bg-amber-800 text-white font-bold py-2 px-4 rounded hover:bg-amber-600 transition duration-300"> Limpiar filtros</button>
        </div>
