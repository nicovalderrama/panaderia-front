...
export default function ComprobantePage() {
  ...
  const generarPDF = () => {
    ...
    ventaData.items.forEach(item => {
      ...
      doc.text(`$${item.producto_precio_unidad}`, 120, y)
      doc.text(item.tipo_precio === 'mayorista' ? 'Mayorista' : 'Lista', 160, y)
    })
  ...
  //     'Precio': item.producto_precio_unidad,
  //     'Tipo de Precio': item.tipo_precio === 'mayorista' ? 'Mayorista' : 'Lista',
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <div className="flex flex-col justify-center bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
          ...
          <div className="mb-4">
            ...
            <p className="text-lg"><span className="font-semibold">Número comprobante:</span> {ventaData.numero_comprobante}</p>
            ...
            <p className="text-lg"><span className="font-semibold">Usuario venta:</span> {ventaData.user_name_venta}</p>
          ...
          <div className="border-t border-b border-[#8B4513] py-4 mb-4">
            {ventaData.items.map((item, index) => (
              <div key={index} className="flex justify-between mb-2">
                ...
                <span>${item.producto_precio_unidad} ({item.tipo_precio === 'mayorista' ? 'Mayorista' : 'Lista'})</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col space-y-2">
            ...
            {/* <button
              ...
              className="w-full bg-[#006400] text-white font-bold py-2 px-4 rounded hover:bg-[#008000] transition duration-300"
            >
              Descargar Excel
            </button> */}
          </div>
