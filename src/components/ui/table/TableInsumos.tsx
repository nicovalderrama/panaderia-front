...
export const TableInsumos = ({ headers, data, actions }: TableProps) => {
  return (
        ...
        <tbody>
          {data.map((row, rowIndex) => (
            <tr key={rowIndex} className="bg-white border-b hover:bg-gray-200">
              ...
              <td className="px-6 py-4">{row.stock_actual}</td>
              <td className="px-6 py-4">{row.punto_pedido}</td>
              <td className="px-6 py-4">{row.proveedor_frecuente}</td>
              <td className="px-6 py-4">{row.precio_comprado}</td>
